 ...
 *   Callbacks   — embedder supplies MTLTextures; we wrap each one in a
 *                 private CAMetalDrawable-conforming object whose
 *                 -present invokes the embedder's present callback (the
 *                 same shape as D3DMetal's own NeptuneCAMetalDrawable,
 *                 which its present path expects).
 *
 ...
 * completed distinction for embedder textures). */
...
- (void)presentAtTime:(CFTimeInterval)presentationTime {
    ...
    if (_cb.present)
        _cb.present(_cb.ctx, (uint32_t)_drawableID);
...
DmnGFXTSwapchain::DmnGFXTSwapchain(void* mtlDevice)
    : mtl_device_([(id<MTLDevice>)mtlDevice retain]),
      last_drawable_(nullptr), present_layer_(nullptr), present_cb_{} {}
...
bool DmnGFXTSwapchain::InitializeForHWND(void* hwnd,
    ...
    if (w->backend == DmnWindowBackend::Callbacks) {
        if (w->cb.configure &&
            !w->cb.configure(w->cb.ctx, width, height, (uint32_t)format,
                             desc->BufferCount)) {
            DMN_ERROR("InitializeForHWND(%p): embedder configure() failed",
                      hwnd);
            return false;
        }
        /* Keep our own copy of the callbacks so late presents (after
         * dmn_window_destroy) skip the hwnd lookup — the callback-backend
         * analog of present_layer_ below. */
        present_cb_ = w->cb;
    } else {
...
void* DmnGFXTSwapchain::GetDrawableForHWND(void* hwnd) {
    ...
    @autoreleasepool {
         ...
         * or the copied embedder callbacks): both outlive the app's window
         * handle, so an async present that fires after dmn_window_destroy
         * still resolves a drawable instead of aborting D3DMetal on a failed
        ...
        if (layer) {
        ...
        } else if (present_cb_.acquire_texture) {
            uint32_t slot = 0;
            id<MTLTexture> tex = (id<MTLTexture>)present_cb_.acquire_texture(
                present_cb_.ctx, &slot);
            if (!tex) {
                DMN_WARN("GetDrawableForHWND(%p): embedder returned no "
                         "texture", hwnd);
                return nullptr;
            }
            ...
            DmnWindow* w = dmn_window_lookup(hwnd);
            if (!w) {
                DMN_WARN("GetDrawableForHWND(%p): unknown hwnd", hwnd);
                return nullptr;
            }
            if (w->backend == DmnWindowBackend::Callbacks) {
                uint32_t slot = 0;
                id<MTLTexture> tex =
                    (id<MTLTexture>)w->cb.acquire_texture(w->cb.ctx, &slot);
                if (!tex) {
                    DMN_WARN("GetDrawableForHWND(%p): embedder returned no "
                             "texture", hwnd);
                    return nullptr;
...
bool DmnGFXTSwapchain::ResizeBacking(void* hwnd, uint32_t width,
    ...
    if (w->backend == DmnWindowBackend::Callbacks) {
        if (w->cb.configure &&
            !w->cb.configure(w->cb.ctx, width, height, (uint32_t)resolved, 0))
            return false;
    } else {
        CAMetalLayer* layer = w->layer;
        if (format != DXGI_FORMAT_UNKNOWN) {
...
bool DmnGFXTSwapchain::Present(void* hwnd) {
    /* D3DMetal presents real CAMetalDrawables through its own command
     * buffers; this host-side hook only needs to succeed. The fallback
