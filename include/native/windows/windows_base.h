...
#ifdef __OBJC__
#error "windows_base.h must not be included from Objective-C(++) translation units \
(the BOOL typedef conflicts with objc.h); use d3dmetal_native.h or src/dmn_directx_types.h instead"
#endif
...
/* d3dmetal-native: WCHAR must be 16-bit to match D3DMetal.framework's
 * Windows-shaped ABI (e.g. DXGI_ADAPTER_DESC::Description[128]); macOS
 * wchar_t is 32-bit. Use u"..." literals, never L"...". */
#ifdef __cplusplus
...
/* d3dmetal-native: D3DMetal.framework's D3D/DXGI/COM surface is compiled
 * with the Microsoft x64 calling convention (so Wine PE code can call it
 * without thunks) — verified from the binary: CreateDXGIFactory1 takes its
 * arguments in %rcx/%rdx and every method saves xmm6-15 (MS callee-saved).
 * All COM methods and D3D entry points must therefore be ms_abi. The GFXT
 * host interface is plain SysV and unaffected. */
