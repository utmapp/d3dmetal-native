 ...
 * the library renders into embedder-supplied MTLTextures
 * (dmn_window_create_with_callbacks) and a render thread blits the latest
 * presented texture into the MTKView's drawable each vsync. Frame pacing
 * comes from an in-flight budget in acquire_texture plus the MTKView
 * drawable pool, mirroring CAMetalLayer's nextDrawable backpressure.
 * BGRA8-format swapchains only (what all the demos use). */
 ...
 * for the callback-backed swapchain). */
